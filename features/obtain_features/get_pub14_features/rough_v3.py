...
def find_substring_in_string(tokenizer, text, substring, model_name, at_start_of_sentence=False):
    ...
    print(token_indices[0], token_indices[-1]) # ==> when this is the same, see below for examples

    if token_indices[0] == token_indices[-1]:

        if token_indices[0] == 0:
            pass
        else:
            start, end = token_indices[0] - 2, token_indices[-1] -1
    else:
...
def check_if_phrase_at_the_end(main_string, suffix):
    cleaned_string = main_string.rstrip('.')  # removes trailing full stop(s)
    return cleaned_string.endswith(suffix)

# # Example usage
# sentence = "This is a test sentence"
# print(ends_before_period(sentence + ".", "sentence"))  # True
# print(ends_before_period(sentence + ".", "test"))      # False
...
class Features_Calculator:
    ...
    def boundary(self):
        ...
        position_first_word_after_expression = self.phrase_index[-1]

        position_token_at_end_of_expression = position_first_word_after_expression - 1

        print(f"position_first_word_after_expression: {position_first_word_after_expression}")

        sentence_values = self.data_dict["sentence"]
        print(f'len(sentence_values): {len(sentence_values)}')
        print()
...
if __name__ == "__main__":
    ...
    for pretext, options, correct_answer, id, referenced_term, correct_answer in tqdm(sentence_choices):
        ...
        phrase_at_end_of_sentence = False
        ...
        elif check_if_phrase_at_the_end(cleaned_sentence, referenced_term):
            print("!!! Phrase at the end of the sentence - will add space to refereced term")
            referenced_term = " " + referenced_term
            print(f"new referenced_term: {referenced_term}")
            phrase_at_end_of_sentence = True
        else:
            pass
        ...
        for metric in metrics:
            ...
            if referenced_term == " ride":
                phrase_index = (phrase_index[0]-1, phrase_index[1]-1)
            else:
                pass
            
            if phrase_index == (0, 0) and referenced_term == " England":
                phrase_index = (0, 1)

            if phrase_index[0] == phrase_index[1]:
                phrase_index = (phrase_index[0], phrase_index[1] + 1)

            if referenced_term == " walk up the aisle":
                phrase_index = (5, 9)
            # elif referenced_term == " plates" or referenced_term == " bowl" or referenced_term == " fifty":
            #     phrase_index = (phrase_index[0]-1, phrase_index[1]-1)
