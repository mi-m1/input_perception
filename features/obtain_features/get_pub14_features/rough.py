...
def find_substring_in_string(tokenizer, text, substring):
...
def extract_term(question):
    ...
    # Match various quote and whitespace scenarios
    match = re.search(r'What does (?:the\s+)?["]*\s*(.*?)\s*["]* refer to\s*\?', question, re.IGNORECASE)
    return match.group(1) if match else None


# Examples
print(extract_term('What does the " head" refer to ? '))              # Output: XXX
print(extract_term('What does "" XXXX "" refer to?'))       # Output: XXXX
...
if __name__ == "__main__":
    ...
    # start of the loop
    for pretext, options, correct_answer, id, referenced_term, correct_answer in sentence_choices:
        ...
        for metric in metrics:
            ...
            extractor = Extractor(
                ...
                extractedPhrase=referenced_term,
            ...
            print(f"Phrase index (main part): {phrase_index}")

            # if phrase_index == (None, None):
            #     print("Phrase not found in sentence values. Skipping this entry.")
            #     continue
            # else:
            ...
            print(f"context values (main part): {context_values}")
