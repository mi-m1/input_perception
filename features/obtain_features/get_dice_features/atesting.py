...
class Extractor:
    ...
    def __init__(self, tokenizer, model_name, query, cleaned_sentence, extractedPhrase, calc_func, start_of_sentence_token, model, gamma,):
...
class Features_Calculator:
    ...
    def boundary(self):
        ...
        print(f"{sentence_values[position_token_at_end_of_expression]}")
        print(f"{sentence_values[position_first_word_after_expression]}")
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(args.path_to_dice).iloc[:20]

    idiom_sent_occurrence= list(zip(df.ID, df.Idiom, df.Sentence_Cleaned, df.Extracted_Phrase, df.label))
    ...
    for id, base_exp, cleaned_sentence, extractedPhrase, label in tqdm(idiom_sent_occurrence):
        ...
        metrics = [surprisal, cws, vanilla_entropy, cis,]
    ...
    #write all to .csv
    path_to_save = f"features/{args.hf_model}/"
